...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # owned by default. A test/dev parent may nevertheless carry a
            # group the caller is not a member of (for example /private/tmp
            # is wheel-owned on macOS), which makes chmod silently clear the
        ...
        # requested mode only after ownership is final.
        path.chmod(mode)
