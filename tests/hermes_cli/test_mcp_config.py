...
class TestMcpLogin:
    @pytest.mark.parametrize("configured_flow, requested_flow", [("device", "browser"), ("browser", "device")])
    def test_login_flow_override_controls_authorization_and_probe(
        self, tmp_path, monkeypatch, configured_flow, requested_flow
    ):
        _seed_config(tmp_path, {
            "fixture": {"url": "https://example.com/mcp", "auth": "oauth",
                        "oauth": {"flow": configured_flow}},
        })
        device_calls, probe_flows = [], []

        async def device_login(name, url, cfg):
            device_calls.append(name)

        def probe(name, cfg, **kwargs):
            probe_flows.append(cfg["oauth"]["flow"])
            return []

        monkeypatch.setattr("tools.mcp_oauth_device.login_device", device_login)
        monkeypatch.setattr("tools.mcp_device_oauth.run_device_authorization",
                            lambda name, *args, **kwargs: device_calls.append(name))
        monkeypatch.setattr("hermes_cli.mcp_config._probe_single_server", probe)
        monkeypatch.setattr("hermes_cli.mcp_config._oauth_tokens_present", lambda name: True)

        from hermes_cli.mcp_config import cmd_mcp_login
        cmd_mcp_login(_make_args(name="fixture", flow=requested_flow))

        assert device_calls == (["fixture"] if requested_flow == "device" else [])
        assert probe_flows == [requested_flow]
