...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    sequence (e.g. a non-UTF-8 locale, or an embedded terminal). The two
    interactive update prompts call input() directly -- config migration
    and stash restore -- and each must fail safe (skip, don't crash) rather
    than let the exception escape and crash `hermes update` mid-flight.
    """
