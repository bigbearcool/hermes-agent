...
def test_timeout_kills_descendants(tmp_path):
    ...
    t0 = time.monotonic()
    # Give the shell enough time to start under the full suite's 20-worker
    # load before exercising timeout cleanup; the assertion below still
    # verifies that the timeout path kills the entire process group.
    r = _spawn(_spec(str(script), timeout=3), "{}")
